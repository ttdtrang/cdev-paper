...
p2_mse_vs_df.percentDE = ggplot(performance.df) +
    geom_point(aes(x=diffFraction,y=mse, color=percentDE, group=percentDE), alpha = 0.3) +
    theme_bw(base_size = THEME_BASE_SIZE)
# ggplot(performance.df) +
#     geom_point(aes(x=diffFraction,y=AUC, color=percentDE, group=percentDE), alpha = 0.3) +
#     theme_bw(base_size = THEME_BASE_SIZE)
...
p2_mse_vs_auc.percentDE = ggplot(performance.df) +
    geom_point(aes(x=mse,y=AUC, color=percentDE, group=percentDE), alpha = 0.3) +
    theme_bw(base_size = THEME_BASE_SIZE) +
    ylim(c(0,1))
...
```{r,fig.width=8,fig.height=15}
egg::ggarrange(pl_mse_vs_df.ngene + theme(legend.position = 'none',axis.title.x=element_blank()),pl_mse_vs_auc.ngene + theme(axis.title.x=element_blank()),
